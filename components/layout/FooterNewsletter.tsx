...
export function FooterNewsletter() {
  ...
  if (status === 'done') {
    return (
      <p className="flex items-center gap-2 font-body text-[0.85rem] text-[#F5F3ED]/70 md:justify-self-end">
    ...
    <form onSubmit={handleSubmit} className="w-full md:max-w-sm md:justify-self-end" noValidate>
      <div className="flex items-center gap-3 border-b border-[#C9A96E]/30 focus-within:border-[var(--color-lp-gold)] transition-colors duration-300 pb-2">
        <input
          ...
          className="flex-1 min-w-0 bg-transparent font-body text-[0.85rem] text-[var(--color-lp-porcelain)] placeholder:text-[#F5F3ED]/35 outline-none py-1.5"
        />
        <button
          type="submit"
          disabled={pending}
          aria-label="Subscribe to the newsletter"
          className="text-[#F5F3ED]/55 hover:text-[var(--color-lp-gold)] transition-colors duration-200 disabled:opacity-40"
        >
